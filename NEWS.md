...
- The new argument `feature_names` allows to specify the feature names to calculate SHAP values for. The default equals to `colnames(X)`. This should be changed only in situations when `X` (the dataset to be explained) contains non-feature columns.
- The background dataset is can now consist of a single row. This is useful in situations with natural "off" value such as for MNIST data or for models that can naturally deal with missing values.
